...
function createSensorStatus(egoSpeed, vehicles, pedestrians) {
  ...
  return {
    lidar: { status: 'active', confidence: parseFloat((baseConfidence + Math.random()).toFixed(1)), fps: 10, range: 200, pointsPerScan: 64000 },
    camera: { status: 'active', confidence: parseFloat((baseConfidence - 2 + Math.random() * 2).toFixed(1)), fps: 30, resolution: '1920x1080' },
    radar: { status: 'active', confidence: parseFloat((baseConfidence + 0.5 + Math.random()).toFixed(1)), range: 250, updateRate: 20 },
    ultrasonic: { status: 'active', confidence: parseFloat((98 + Math.random()).toFixed(1)), range: 8, sensors: 12 },
    gps: { status: 'active', accuracy: parseFloat((1 + Math.random()).toFixed(1)), satellites: 12, hdop: parseFloat((0.7 + Math.random() * 0.3).toFixed(1)) },
    fusion: { status: 'active', confidence: parseFloat((baseConfidence - 1).toFixed(1)), latency: 45, tracking: vehicles.length + pedestrians.length },
...
export default function Dashboard({ onBack }) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      state.vehicles = state.vehicles.map(v => {
        ...
        if (newY > 150 || newY < 8) {
          return { ...v, y: 40 + Math.random() * 80, speed: state.egoSpeed + (Math.random() - 0.5) * 20, lane: LANES[Math.floor(Math.random() * LANES.length)], vehicleType: VEHICLE_TYPES[Math.floor(Math.random() * VEHICLE_TYPES.length)] }
        }
        return { ...v, y: newY }
      })
      ...
      if (state.vehicles.length < 4 && Math.random() < 0.02) {
        ...
        if (!state.vehicles.some(v => Math.abs(v.y - newVehicle.y) < 20 && v.lane === newVehicle.lane)) {
      ...
      state.pedestrians = state.pedestrians.map(p => {
        ...
        if (p.state === 'approaching') {
          newX += p.speed * dt * 3
          if (newX > -3) { newState = 'crossing'; newDirection = Math.random() > 0.4 ? 'crossing' : 'walking' }
        } else if (p.state === 'crossing') {
          newY += p.speed * dt * 3
          newX += (p.direction === 'right' ? 1 : -0.3) * p.speed * dt * 2
          if (newY > 100 || Math.abs(newX) > 20) return null
          if (Math.random() < 0.01) newDirection = Math.random() > 0.5 ? 'left' : 'right'
        } else if (p.state === 'walking') {
          newY += p.speed * dt * 2
          newX += (p.direction === 'right' ? 1 : -1) * p.speed * dt
          if (newY > 100 || Math.abs(newX) > 25) return null
        }

        return { ...p, x: newX, y: newY, state: newState, direction: newDirection, speed: newSpeed }
      }).filter(Boolean)
      ...
      const lidarPoints = generateLidarPoints(state.vehicles, state.pedestrians)

      let detections = []
      if (now - state.lastDetectionTime > 200) {
      ...
      if (now - state.lastConfidenceTime > 100) {
        const avgConf = detections.length > 0 ? detections.reduce((sum, d) => sum + d.confidence, 0) / detections.length : 96.8
  ...
  const perspectiveVehicle = (v) => {
    ...
    return (
      <motion.div key={v.id} className="absolute" style={{ left: `${(perspectiveX / 400) * 100}%`, top: `${(perspectiveY / 240) * 100}%`, width: vehicleWidth, height: vehicleHeight }} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 text-[7px] text-white/50 whitespace-nowrap">{v.y.toFixed(0)}m</div>
      </motion.div>
    )
  }
  ...
  const perspectivePedestrian = (p) => {
    ...
    return (
      <motion.div key={p.id} className="absolute" style={{ left: `${(perspectiveX / 400) * 100}%`, top: `${(perspectiveY / 240) * 100}%`, width: personWidth, height: personHeight }} animate={p.state === 'approaching' ? { y: [0, -1.5, 0] } : {}} transition={p.state === 'approaching' ? { duration: 0.4, repeat: Infinity } : {}}>
  ...
  return (
    <motion.div className="min-h-screen bg-black road-grid p-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        ...
        <div className="col-span-3 space-y-6">
            ...
            <div className="space-y-3">
                ...
                return (
                  <div key={sensor} className="p-3 rounded-xl bg-white/5">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-3"><div className={`w-2.5 h-2.5 rounded-full ${data.status === 'active' ? 'bg-av-success animate-pulse' : 'bg-av-danger'}`} /><span className="text-sm text-white capitalize">{sensor}</span></div>
                      <span className="text-xs font-mono text-av-primary">{data.confidence?.toFixed(1)}%</span>
                    </div>
                    <div className="h-1 bg-white/10 rounded-full overflow-hidden"><motion.div className="h-full bg-gradient-to-r from-av-primary to-av-accent rounded-full" animate={{ width: `${data.confidence || 0}%` }} transition={{ duration: 0.5 }} /></div>
                  </div>
                )
              })}
              {simulationData.sensorStatus?.fusion && (
                <div className="p-3 rounded-xl bg-av-primary/10 border border-av-primary/20">
                  ...
                  <div className="flex gap-3 text-[10px] text-white/40"><span>Latency: {simulationData.sensorStatus.fusion.latency}ms</span><span>Tracking: {simulationData.sensorStatus.fusion.tracking} objects</span></div>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="glass-panel p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-white/5 text-center"><p className="text-2xl font-bold text-av-primary">{Math.round(simulationData.egoSpeed * 0.621)}<span className="text-sm text-white/30 ml-1">mph</span></p><p className="text-xs text-white/50">Ego Speed</p></div>
              <div className="p-4 rounded-xl bg-white/5 text-center"><p className="text-2xl font-bold text-av-danger">{threatCounts.total}</p><p className="text-xs text-white/50">Threats</p></div>
              <div className="p-4 rounded-xl bg-white/5 text-center"><p className="text-2xl font-bold text-av-success">{detectionStats.vehicles}</p><p className="text-xs text-white/50">Vehicles</p></div>
              <div className="p-4 rounded-xl bg-white/5 text-center"><p className="text-2xl font-bold text-av-warning">{detectionStats.pedestrians}</p><p className="text-xs text-white/50">Pedestrians</p></div>
            </div>
          </div>
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={threat.name} className="flex items-center justify-between p-3 rounded-xl bg-white/5">
                  <div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${threat.color}`} /><span className="text-xs text-white/70">{threat.name}</span></div>
        ...
        <div className="col-span-6 space-y-6">
          <div className="glass-panel p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2"><span className={`w-2 h-2 rounded-full ${isMonitoring ? 'bg-av-success animate-pulse' : 'bg-av-danger'}`} /><span className="text-xs text-white/50">{isMonitoring ? 'Live Feed' : 'Paused'}</span></div>
            </div>
            <div className="relative">
              ...
              <div className="absolute top-4 left-4 flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/60 border border-white/10">
                <span className="text-xs text-white/50">Range:</span><span className="text-xs text-av-primary font-mono">150m</span>
              </div>
              <div className="absolute bottom-4 right-4 flex gap-4 text-xs">
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-av-primary" /><span className="text-white/50">LIDAR</span></div>
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-av-success" /><span className="text-white/50">Vehicle</span></div>
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-av-warning" /><span className="text-white/50">Pedestrian</span></div>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`text-2xl font-bold ${currentConfidence < 85 ? 'text-av-warning' : 'text-av-success'}`}>{currentConfidence.toFixed(1)}%</span>
            </div>
            <div className="h-[150px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={confidenceHistory}>
                  <defs><linearGradient id="confGradient" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#06b6d4" stopOpacity={0.3} /><stop offset="100%" stopColor="#06b6d4" stopOpacity={0} /></linearGradient></defs>
                  <XAxis dataKey="time" hide />
                  <YAxis domain={[70, 100]} hide />
                  <Area type="monotone" dataKey="value" stroke="#06b6d4" strokeWidth={2} fill="url(#confGradient)" isAnimationActive={false} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="col-span-3 space-y-6">
          <div className="glass-panel p-6">
            ...
            <div className="relative aspect-video bg-gradient-to-b from-sky-900 via-sky-700 to-gray-800 rounded-xl overflow-hidden">
              ...
              <svg className="absolute top-[10%] w-full h-[10%]" viewBox="0 0 400 60" preserveAspectRatio="none"><polygon points="0,60 0,30 20,40 40,25 60,45 80,35 100,50 120,30 150,45 180,20 220,40 260,25 300,45 350,30 400,50 400,60" fill="rgba(0,0,0,0.3)" /></svg>
              <svg className="absolute inset-0 w-full h-full" viewBox="0 0 400 240">
                <defs>
                  <linearGradient id="roadPerspective" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#2a2a2a" /><stop offset="100%" stopColor="#1a1a1a" /></linearGradient>
                  <linearGradient id="grassGrad" x1="0%" y1="0%" x2="0%" y2="100%"><stop offset="0%" stopColor="#1a3d1a" /><stop offset="100%" stopColor="#0a1f0a" /></linearGradient>
                </defs>
                ...
                <g opacity="0.4">{[0, 15, 30, 45, 60, 75, 90, 105, 120, 135, 150, 165, 180].map((offset, i) => (<rect key={i} x={offset} y="200" width="8" height="4" fill="white" />))}</g>
              </svg>
              ...
              <div className="absolute bottom-2 left-1/2 -translate-x-1/2 text-[10px] text-white/40">{Math.round(simulationData.egoSpeed * 0.621)} MPH</div>
              <div className="absolute top-2 left-2 flex items-center gap-1 px-2 py-1 rounded bg-black/60">
                <div className={`w-2 h-2 rounded-full ${isMonitoring ? 'bg-av-success animate-pulse' : 'bg-av-danger'}`} /><span className="text-[10px] text-white/70">CAM FRONT</span>
              </div>
              <div className="absolute top-2 right-2 flex items-center gap-1 px-2 py-1 rounded bg-av-danger/20 border border-av-danger/30">
                <div className="w-2 h-2 rounded-full bg-av-danger animate-pulse" /><span className="text-[10px] text-av-danger">REC</span>
              </div>
            </div>
          </div>
          ...
          <div className="glass-panel p-6 flex-1">
            ...
            <div className="space-y-2 max-h-[350px] overflow-y-auto">
              {simulationData.detections.slice(0, 15).map((det) => (
                <motion.div key={det.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className={`p-3 rounded-xl text-xs ${det.threat ? 'bg-av-danger/10 border border-av-danger/30' : 'bg-white/5 border border-white/10'}`}>
                  ...
                  {det.threat && (
                    <div className="mt-2 flex items-center gap-1 text-av-danger">
                      <svg className="w-3 h-3" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L1 21h22L12 2zm0 3.5L19.5 19h-15L12 5.5zM11 10v4h2v-4h-2zm0 6v2h2v-2h-2z" /></svg>
              ...
              {simulationData.detections.length === 0 && (
                <div className="text-center text-white/30 py-8">Scanning environment...</div>
              )}
            </div>
          </div>
        </div>
